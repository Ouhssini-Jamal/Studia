...
<body onload="" class="view-body">
          ...
          <div class="" >@include('components.side-bar')</div> 
            ...
            @if($classrooms->count()==0)
                ...
                @if ($message = Session::get('success'))
                        <div class="alert mx-auto w-50 alert-success alert-block">
    ...
    background-size: cover;">
                       ...
                       @if($classroom->is_public === 0)
                       <h3><Strong style="color:white;background-color: #3e606091;">Classroom code: </Strong><span style="color:white;color:white;background-color: #3e606091;">{{$classroom->code}}</span></h3>
                       @else <h3><Strong  style="color:white;background-color: #3e606091;">Public</Strong></h3>
                       @endif
                       <div class="dropdown-menu dropdown-menu-end">
                        <form action="{{route('destroy', ['id' => $classroom->id])}}" id="myform" method="POST">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }} 
                ...
                @else<h2 style="box-shadow: 0 4px 8px 0 rgb(0 0 0 / 20%)" class="p-2">You have no classroom !!<br>Create one</h2>
                @endif
            </div>
            <script>
                function openNav() {
